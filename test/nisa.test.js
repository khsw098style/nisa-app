...
function parseRakutenCSVText(text, dataDate) {
  ...
  return {
    ...
    importedAt: dataDate || '2026/05/10',
  };
}
...
describe('ファイル名から日付を取得', () => {
  test('正常なファイル名から日付を取得できる', () => {
    const date = extractDateFromFilename('assetbalance_all__20260510_221127.csv');
    expect(date).toBe('2026/05/10');
  });

  test('別の日付でも正しく取得できる', () => {
    const date = extractDateFromFilename('assetbalance_all__20261231_093045.csv');
    expect(date).toBe('2026/12/31');
  });

  test('日付パターンがない場合は今日の日付を返す', () => {
    const date = extractDateFromFilename('unknown_file.csv');
    expect(date).toBeTruthy(); // 今日の日付が返る
  });

  test('CSVインポート時にファイル名の日付がimportedAtに反映される', () => {
    const csv = makeCSVRow('NISAつみたて投資枠', '17,614', '34,063.81', '36,326', '63,985', '+3,985', '+6.64');
    const dataDate = extractDateFromFilename('assetbalance_all__20260510_221127.csv');
    const result = parseRakutenCSVText(csv, dataDate);
    expect(result.importedAt).toBe('2026/05/10');
  });
});
